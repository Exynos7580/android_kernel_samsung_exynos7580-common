...
DEFINE_EVENT(set, cpufreq_cafactive_cpu_min_qos,
        TP_PROTO(u32 cpu_id, unsigned long targfreq,
             unsigned long actualfreq),
        TP_ARGS(cpu_id, targfreq, actualfreq)
);

DEFINE_EVENT(set, cpufreq_cafactive_cpu_max_qos,
        TP_PROTO(u32 cpu_id, unsigned long targfreq,
             unsigned long actualfreq),
        TP_ARGS(cpu_id, targfreq, actualfreq)
);
#if defined(CONFIG_ARM_EXYNOS_MP_CPUFREQ) || defined(CONFIG_ARM_EXYNOS_SMP_CPUFREQ)
DEFINE_EVENT(set, cpufreq_cafactive_kfc_min_qos,
        TP_PROTO(u32 cpu_id, unsigned long targfreq,
             unsigned long actualfreq),
        TP_ARGS(cpu_id, targfreq, actualfreq)
);

DEFINE_EVENT(set, cpufreq_cafactive_kfc_max_qos,
        TP_PROTO(u32 cpu_id, unsigned long targfreq,
             unsigned long actualfreq),
        TP_ARGS(cpu_id, targfreq, actualfreq)
);

...
static int max86900_gpio_setup(struct max86900_device_data *data)
{
	...
	errorno = gpio_request(data->vdd_1p8_gpio, "hrm_vdd");
	if (errorno) {
		pr_err("%s - failed to request hrm_vdd\n", __func__);
		return errorno;
	}
	gpio_direction_output(data->vdd_1p8_gpio, HRM_LDO_OFF);

	errorno = gpio_request(data->led_3p3_gpio, "hrm_led");
	if (errorno) {
		pr_err("%s - failed to request hrm_led\n", __func__);
		return errorno;
	}
	gpio_direction_output(data->led_3p3_gpio, HRM_LDO_OFF);

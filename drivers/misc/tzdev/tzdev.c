...
/* migrate_pages() is significantly different between Linux
 * versions so, we need to separate it for each version but,
 * currently, we consider only both 3.10.xx and 3.4.xx */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
#define MIGRATE_PAGES(list, alloc)	migrate_pages((list), (alloc), 0, MIGRATE_SYNC, MR_MEMORY_FAILURE)
#else
#define MIGRATE_PAGES(list, alloc)	migrate_pages((list), (alloc), 0, false, MIGRATE_SYNC)
#endif
...
static int tzdev_migrate_pages(struct task_struct *task, struct mm_struct *mm,
		unsigned long start, unsigned long nr_pages, int write, struct page **pages)
{
	int res, i;
	LIST_HEAD(mem_pages_list);

	res = tzdev_get_user_pages(task, mm, start, nr_pages, write, 0, pages, NULL);
	if (res)
		return res;
	...
	migrate_prep();
	for (i = 0; i < nr_pages; i++) {
		if (get_pageblock_migratetype(pages[i]) != MIGRATE_CMA)
			continue;
	...
	res = MIGRATE_PAGES(&mem_pages_list, tzdev_alloc_kernel_page);
	if (res) {
...
#if defined(CONFIG_TZDEV_PAGE_MIGRATION)
	res = tzdev_migrate_pages(task, mm, (unsigned long)s->ptr,
...
#if defined(CONFIG_TZDEV_PAGE_MIGRATION)
	res = tzdev_verify_migration(pages, nr_pages);
	if (res) {
		tzdev_print(0, "Failed migration. Some pages are still in CMA\n");
		ret = res;
		goto out_pin;
	}
#endif

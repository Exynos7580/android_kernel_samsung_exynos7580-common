...
#define FW_IMAGE_NAME_D1	"tsp_stm/stm_s.fw"
#define FW_IMAGE_NAME_D2	"tsp_stm/stm_t.fw"
...
int fts_fw_update_on_probe(struct fts_ts_info *info)
{
	...
	else if (info->digital_rev == 1)
		info->firmware_name = FW_IMAGE_NAME_D1;
	else if (info->digital_rev == 2)
		info->firmware_name = FW_IMAGE_NAME_D2;
	...
	if (info->digital_rev == 1 && fw_entry->size!=(FW_IMAGE_SIZE_D1 + sizeof(struct fts64_header))) {
		tsp_debug_err(true, info->dev,
	...
	if (info->digital_rev == 2 && fw_entry->size!=(FW_IMAGE_SIZE_D2 + sizeof(struct fts64_header))) {
		tsp_debug_err(true, info->dev,

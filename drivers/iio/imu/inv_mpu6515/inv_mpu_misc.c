...
#define DMP_IMAGE_CRC_VALUE             0xbb6ce4e9
...
inv_error_t inv_set_pedometer_step_threshold(struct inv_mpu_state *st, int threshold)
{
	...
	data[0] = (threshold >> 8) & 0xff;
	data[1] = threshold & 0xff;
	result = mem_w_key(KEY_D_PEDSTD_SB, 2, data);
	if (result)
	{
		pr_err("%s Fail to write PEDSTD_SB.\n", __func__);
		return INV_ERR_DMP_REG;
	}
	//result = write_be32_to_mem(st, PEDO_PEAK_THRESHOLD, PEDSTD_PEAKTHRSH);
	data[0] = (PEDO_PEAK_THRESHOLD >> 24) & 0xff;
	data[1] = (PEDO_PEAK_THRESHOLD >> 16) & 0xff;
	data[2] = (PEDO_PEAK_THRESHOLD >>  8) & 0xff;
	data[3] = (PEDO_PEAK_THRESHOLD)       & 0xff;
	result = mem_w_key(KEY_D_PEDSTD_PEAKTHRSH, 4, data);
	if (result)
	{
		pr_err("%s Fail to write PEDSTD_PEAKTHRSH.\n", __func__);
		return INV_ERR_DMP_REG;
	}
	//result = inv_write_2bytes(st, PEDSTD_SB_TIME, 150);
	data[0] = (150 >> 8) & 0xff;
	data[1] = 150 & 0xff;
	result = mem_w_key(KEY_D_PEDSTD_SB_TIME, 2, data);
	if (result)
	{
		pr_err("%sFail to write PEDSTD_SB_TIME.\n", __func__);
		return INV_ERR_DMP_REG;
	}

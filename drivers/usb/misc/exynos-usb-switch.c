...
static void set_host_vbus(struct exynos_usb_switch *usb_switch, int value)
{
	...
	} else if (!IS_ERR(usb_switch->vbus_reg)) {
		int ret;

		if (value)
			ret = regulator_enable(usb_switch->vbus_reg);
		else
			ret = regulator_disable(usb_switch->vbus_reg);

		if (ret)
			pr_err("Failed to turn UHost Vbus %s\n",
					value ? "on" : "off");
	}
}
				...
				enum usb_cable_status mode)
{
	...
	case USB_HOST_DETACHED:
		...
		if (usb_switch->ohci_rh_dev)
			pm_runtime_allow(usb_switch->ohci_rh_dev);
		if (usb_switch->ehci_rh_dev)
			pm_runtime_allow(usb_switch->ehci_rh_dev);
	...
	case USB_HOST_ATTACHED:
		...
		if (usb_switch->ehci_rh_dev)
			pm_runtime_forbid(usb_switch->ehci_rh_dev);
		if (usb_switch->ohci_rh_dev)
			pm_runtime_forbid(usb_switch->ohci_rh_dev);
		if (usb_switch->gpio_host_vbus)
				     ...
				     struct device *dev)
{
	...
	if (!node) {
		...
		if (!pdev) {
			...
			usb_switch->ehci_driver = NULL;
			hcd = platform_get_drvdata(pdev);
			if (hcd) {
				usb_switch->ehci_rh_dev = &hcd->self.root_hub->dev;
				usb_switch->ehci_driver = container_of(usb_switch->ehci_dev->driver,
						struct platform_driver, driver);
			} else {
				dev_warn(dev, "failed to get driver data for ehci device\n");
			}
		}
		of_node_put(node);
	}
	...
	if (!node) {
		...
		if (!pdev) {
			...
			usb_switch->ohci_driver = NULL;
			exynos_ohci = platform_get_drvdata(pdev);
			if (exynos_ohci) {
				hcd = exynos_ohci->hcd;
				usb_switch->ohci_rh_dev = &hcd->self.root_hub->dev;
				usb_switch->ohci_driver = container_of(usb_switch->ohci_dev->driver,
						struct platform_driver, driver);
			} else {
				dev_warn(dev, "failed to get driver data for ohci device\n");
			}
		}
		of_node_put(node);
	}
...
static int exynos_usbswitch_probe(struct platform_device *pdev)
{
	...
	if (usb_switch->drvdata->driver_unregister)
	{
		if (usb_switch->ohci_driver)
			platform_driver_unregister(usb_switch->ohci_driver);
		if (usb_switch->ehci_driver)
			platform_driver_unregister(usb_switch->ehci_driver);
	}
...
int exynos_otg_vbus_event(struct platform_device *pdev, bool is_active)
{
	struct usb_gadget *gadget = s3c_udc_get_gadget(&pdev->dev);
...
int exynos_id_event(bool id_state)
{
	if (!id_state) {
		wake_lock(&our_switch->wake_lock);
		pr_info("USB_HOST_ATTACHED \n");
		if (our_switch->ehci_driver)
			platform_driver_register(our_switch->ehci_driver);
		if (our_switch->ohci_driver)
			platform_driver_register(our_switch->ohci_driver);

	} else {
		pr_info("USB_HOST_DETACHED \n");
		if (our_switch->ohci_driver)
			platform_driver_unregister(our_switch->ohci_driver);
		if (our_switch->ehci_driver)
			platform_driver_unregister(our_switch->ehci_driver);
		wake_unlock(&our_switch->wake_lock);
	}

	return 0;
}
EXPORT_SYMBOL_GPL(exynos_id_event);
...
struct exynos_usbswitch_drvdata exynos7580_usb_switch_drvdata = {
	.driver_unregister = 1,
	.use_change_mode = 0,
};
...
static const struct of_device_id of_exynos_usbswitch_match[] = {
	{
		...
		.data =	&exynos7580_usb_switch_drvdata,
	},
	{ },
};
